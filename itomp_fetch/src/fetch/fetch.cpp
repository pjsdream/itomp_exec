...
const std::string Fetch::planning_group_ = "arm";
const std::string Fetch::endeffector_name_ = "gripper_link";
const double Fetch::endeffector_vertical_moving_distance_ = 0.10;
const double Fetch::gripper_picking_distance_ = 0.060;
...
void Fetch::initializeTuckState(moveit_msgs::RobotState& start_state)
{
...
void Fetch::initializeDefaultState(moveit_msgs::RobotState& start_state)
{
    start_state.joint_state.name.push_back("shoulder_pan_joint");
    start_state.joint_state.name.push_back("shoulder_lift_joint");
    start_state.joint_state.name.push_back("upperarm_roll_joint");
...
void Fetch::initializeCurrentState(robot_state::RobotState& state)
{
...
void Fetch::initializeCurrentState(moveit_msgs::RobotState& start_state)
{
...
void Fetch::runMovingArmScenario()
{
...
void Fetch::runMovingArmScenarioOMPL()
{
    // prepare for OMPL
...
void Fetch::moveEndeffectorVertically(double distance)
{
...
void Fetch::moveEndeffectorVerticallyTarget(double distance, const Eigen::Vector3d& position, const Eigen::Quaterniond& orientation, bool wait_for_execution)
{
    const double trajectory_duration = 1.0;
    const double planning_timestep = 0.1;
...
void Fetch::moveEndeffectorVerticallyIK(double distance, bool wait_for_execution)
{
    robot_state::RobotState current_state( moveit_robot_model_ );
    initializeCurrentState(current_state);
...
void Fetch::attachSphere(int sphere_index)
{
...
void Fetch::detachSphere(int sphere_index)
{
    const AttachingSphere& sphere = attaching_spheres_[sphere_index];
    const std::string attaching_link_name = sphere.attaching_link;
    const int attaching_link = robot_model_->getJointIndexByLinkName(attaching_link_name);
